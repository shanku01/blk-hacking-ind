...
export function applyTemporalRules(transactions, q, p, k) {
    ...
    for (const tx of txs) {
        ...
        // Apply P sweep
        while (eventIndex < pEvents.length &&
    ...
    const prefix = new Array(remanents.length);
    ...
    const results = kPeriods.map(kp => {
        ...
        const amount = prefix[right] - (left > 0 ? prefix[left - 1] : 0);
        return {
            start: new Date(kp.start).toISOString(),
            end: new Date(kp.end).toISOString(),
            amount
